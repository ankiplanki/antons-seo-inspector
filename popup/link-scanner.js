...
async function startScan() {
  ...
  try {
    ...
    port = api.runtime.connect({ name: "scan" });
...
function saveToSession() {
  ...
  // storage.session has a 10MB quota — skip caching if we'd exceed 9MB
  if (bytes > 9 * 1024 * 1024) return;
  api.storage.session.set({ lastScan: data }).catch(() => {});
}

async function loadFromSession() {
  try {
    const stored = await api.storage.session.get("lastScan");
